...
describe('buildIdpFormData', () => {
  it('includes the required IDP fields and the file', () => {
    const fd = buildIdpFormData(new Uint8Array([1, 2, 3]), 'id.jpg', { filePath: 'id.jpg', docType: 'ID', fileType: 'IMAGE' });
...
describe('executeIdp (ingest → poll)', () => {
  it('ingests with a bearer header then polls the result URL and returns the documents JSON', async () => {
    ...
    const fetchImpl = vi.fn()
      // 1) ingest
      .mockResolvedValueOnce({ ok: true, status: 201, text: async () => JSON.stringify({ data: { request_id: 'req-1', file_ids: ['f.png'] } }) })
      // 2) first poll: not finished
      .mockResolvedValueOnce({ ok: true, status: 200, text: async () => JSON.stringify({ data: { progress: { total_doc: 1, ignored: 0, finished: 0 }, documents: [] } }) })
      // 3) second poll: finished
      .mockResolvedValueOnce({ ok: true, status: 200, text: async () => JSON.stringify({ data: { progress: { total_doc: 1, ignored: 0, finished: 1 }, documents: [{ document_type: 'ID', ocr_data: [{ name: 'name', value: ['A'] }], status: 'SUCCESS' }] } }) });

    const res = await executeIdp({ filePath: '/w/id.jpg', docType: 'ID' }, cfg, { readFile, fetchImpl, sleep: noSleep });

    expect(fetchImpl).toHaveBeenCalledTimes(3);
    // ingest call
    expect(fetchImpl.mock.calls[0][0]).toBe(INGEST);
    expect((fetchImpl.mock.calls[0][1] as RequestInit).method).toBe('POST');
    expect((fetchImpl.mock.calls[0][1] as { headers: Record<string, string> }).headers.Authorization).toBe('Bearer k-123');
  ...
  it('times out gracefully when polling never completes', async () => {
    const readFile = vi.fn().mockResolvedValue(new Uint8Array([1]));
    const fetchImpl = vi.fn()
      .mockResolvedValueOnce({ ok: true, status: 201, text: async () => JSON.stringify({ data: { request_id: 'req-2' } }) })
      .mockResolvedValue({ ok: true, status: 200, text: async () => JSON.stringify({ data: { progress: { total_doc: 1, ignored: 0, finished: 0 }, documents: [] } }) });
