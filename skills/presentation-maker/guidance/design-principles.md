...
`surface` is cards and side panels, `primary` is the single accent,
`text` is body/heading text, `muted` is secondary text and captions.
...
- **Bold coral** (marketing, creative, launch) — bg `FFF7F4`, surface `FCE9E2`, primary `C9341A`, text `2A1D1A`, muted `8A6F68`.
- **Sage calm** (sustainability, wellness, strategy) — bg `F4F7F2`, surface `E4EBDF`, primary `3F6B4A`, text `222A22`, muted `6B7A69`.
...
- Do not invent extra hexes. If you need a lighter shade, use `surface`; if you need emphasis, use `primary`.
...
- Use `surface` panels to group related content, not decorative boxes; leave the rest of the slide as open `bg`.
