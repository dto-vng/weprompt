...
> **Materialized 2026-08-08** from the session-local approved design (unchanged below the line).
> **EPIC-004 status note:** the register's original entry condition — "EPIC-002's OfficeCLI
> ownership contract must settle" — can no longer be satisfied as written: EPIC-002 was re-scoped
> to A0+ (HTML-only) on 2026-08-08 and will not produce an OfficeCLI ownership contract. EPIC-004
...
> Revised: 2026-08-05 — six corrections folded in after a review that probed OfficeCLI 1.0.143 directly. See _Review corrections_ below.  
...
while the existing `callAuthorizedOfficeArtifact` helper _derives_ workspace from the conversation
...
- **Findings are compiler output too**, not model output, wherever they assert workbook facts
  ...
  _reference_ a finding id but never invent one.
...
starves the baseline that every precondition depends on. Only what is _sent to the model_ needs
aggressive capping.
...
| `LOCAL_DIFF_*`     | before/after manifest comparison for the scope gate                        | Must cover the whole workbook, since the gate proves nothing changed _outside_ scope                   |
