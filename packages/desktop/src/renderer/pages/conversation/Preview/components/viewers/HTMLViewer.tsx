...
const HTMLPreview: React.FC<HTMLPreviewProps> = ({ content, file_path, hideToolbar = false }) => {
  ...
  return (
    <div className='h-full w-full flex flex-col bg-1'>
      ...
      {/* 工具栏 */}
      {!hideToolbar && (
        <div className='flex items-center justify-between h-40px px-12px bg-2 border-b border-[var(--border-base)] flex-shrink-0'>
      ...
      <div className='flex-1 flex overflow-hidden'>
        {/* 左侧：代码编辑器（编辑模式时显示） */}
        {editMode && (
          <div className='flex-1 overflow-hidden border-r border-[var(--border-base)]'>
            <MonacoEditor
              height='100%'
              language='html'
      ...
      {contextMenu && (
        <div
          className='fixed bg-1 border border-[var(--border-base)] rd-6px shadow-lg py-4px z-9999'
          style={{
            left: contextMenu.x,
            top: contextMenu.y,
          }}
