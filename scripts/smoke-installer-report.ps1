...
$logPath = Join-Path $env:TEMP "weprompt-installer-smoke-$runId-log.jsonl"
...
@(
  ...
  '{"schemaVersion":1,"event":"failure","session":"smoke","version":"2.1.39","updated":true,"code":"E1003","phase":"atomic-failed","failedPath":"C:\\Users\\employee\\AppData\\Local\\Programs\\WePrompt","blockingProcesses":[],"fallbackReason":"restart-manager-no-process","message":"Windows did not identify a specific locking process. Close terminals, editors, and file managers opened in the install folder."}'
...
- File or folder: C:\Users\employee\AppData\Local\Programs\WePrompt
...
if ($status.status -ne 'exported') {
  throw "Expected exported status, got $($status.status)"
}
if ($status.copyText -notlike '*WePrompt installer failure E1003*') {
  throw 'copyText does not include failure heading'
}
if ([string]::IsNullOrWhiteSpace($status.localExportPath) -or -not (Test-Path -LiteralPath $status.localExportPath)) {
  throw 'Local diagnostic export is missing'
}
...
if (-not (@($events | Where-Object { $_.event -eq 'diagnostics-exported' }).Count -gt 0)) {
  throw 'diagnostics-exported JSONL event missing'
}
...
Write-Output "localExportPath=$($status.localExportPath)"
