...
!macro AIONUI_LOG_EXTRACT_RESULT _METHOD
  ${IfNot} ${FileExists} "$INSTDIR\${APP_EXECUTABLE_FILENAME}"
    !insertmacro AIONUI_FAIL_UX \
      ...
      "event=extract result=fail method=${_METHOD} missing=${APP_EXECUTABLE_FILENAME}" \
      ...
      "extract result=fail method=${_METHOD} missing=${APP_EXECUTABLE_FILENAME} instDir=$INSTDIR" \
      "extract result=fail method=${_METHOD} missing=${APP_EXECUTABLE_FILENAME} instDir=$INSTDIR"
  ${Else}
