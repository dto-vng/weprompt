...
- [ ] **[BUG-031][P2][Creative Studio] Review no longer distinguishes a generating shot from a failed one**
  ...
  - **Next gate:** the `creative-suite-sprint2` → `origin/sprint2` MR. This item stays open until that merge is accepted, per this register's Done rule; the branch is currently 8 commits ahead.
...
- [ ] **[Creative Studio] Review screen redraw — built in full, awaiting integration**
  ...
  - **Current boundary: all five slices are built, reviewed and merged into `creative-suite-sprint2`, which is 9 commits ahead of `origin/sprint2`.** §3a–§3d are complete. The sequencing constraint recorded here is satisfied — `renderCut` reads the cut (R1/R2) and shipped before any editor UI (R3).
  ...
  - **Next gate:** the `creative-suite-sprint2` → `origin/sprint2` MR. Nothing in the redraw is blocked on design.
