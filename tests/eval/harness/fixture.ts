...
import type { EvalDocument, EvalFixture, GoldenQuestion } from './types';
...
export const loadFixture = (): EvalFixture => {
  const documents: EvalDocument[] = readdirSync(CORPUS_DIR)
    .filter((fileName) => fileName.endsWith('.md'))
    .toSorted()
    .map((fileName) => ({ fileName, text: readFileSync(path.join(CORPUS_DIR, fileName), 'utf8') }));
