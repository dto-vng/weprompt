...
- [ ] **[BUG-025][P2][Creative Studio] `StudioPage.dom.test.tsx` batch-action test flakes in full-suite position**
  ...
  - **REPRODUCTION FOUND 2026-08-07** — `bunx vitest run --project dom --coverage tests/unit/pages/studio` fails roughly 2 runs in 3. Coverage instrumentation is the missing ingredient; 35 targeted attempts without it all passed. **Use this command to work the bug.**
  - Mechanism, partially established: at failure the DOM is still showing **Write** (`phase.write.continueToProduce`), so the Produce nav click never navigates — the batch button was never going to exist. `requestTransition` (`StudioPage.tsx`) returns early and silently when a transition is pending, and also aborts after awaiting `flushProjectDraft()` if that fails or `editor.conflict` is set. One of those abort paths is being taken.
  - **Two fixes attempted and reverted, both wrong** — recorded so nobody repeats them. (1) Waiting for the nav entry to be `toBeEnabled()` before clicking: still stranded on Write, so the click is not landing on a disabled control. (2) Making `getProject` return the post-fit project (the fixture returns pre-fit `initial` forever, which looks like a conflicting external edit): did not settle it either, and made a second test in the file — `hides unreachable fit feedback after a canonical routing update refreshes the catalog` — start flaking too, which suggests the two share a cause.
  - **Instrumented 2026-08-07 — the transition machinery is NOT at fault.** Traced every `requestTransition` branch plus each render's `routePhase` under the reproduction. At failure the trace reads `guard-passed to=produce → ready round=0 → navigating to=produce`: no guard rejects, no draft flush fails, no conflict, no `savingBlocked`. **`navigate()` is called and the route does not settle on Produce** — in one captured failure `routePhase` became `produce` for exactly one render and reverted to `write`; in another it never became `produce` at all. The batch button was therefore never going to render, and the DOM correctly shows Write.
  - **Eliminated, with evidence — do not re-try these:** (1) a swallowed click on a disabled nav entry (waiting for `toBeEnabled()` changed nothing, and the guard demonstrably passes); (2) the stale `getProject` fixture returning the pre-fit project (fixing it did not settle the flake and made a second test in the file flake too); (3) `clearWriteFocusIntent` navigating back to a stale `location.pathname` (instrumented — it never fires in the failing run).
  - **Also learned:** the race is timing-fragile enough that `console.error` instrumentation _masks_ it entirely (3/3 passes) — use cheap `globalThis` array pushes instead, dumped from the assertion's catch block. Scope the buffer per test in `beforeEach` or the trace is uselessly cumulative.
  - Next step: the remaining suspect is the test harness's router — why `navigate()` from the transition effect fails to settle `routePhase` under coverage timing. Look at `renderRoute()`'s router setup and whether the navigation needs an `act()` boundary, rather than anything in `StudioPage`'s transition logic.
  - Sightings so far, all full-suite, none reproducible in isolation: two on `creative-suite-sprint2` (loads 15.2 and 7.8), one during a `just push` gate, and one reported by another team on **`sprint2` during a coverage run** (2026-08-07). Two facts follow from the fourth: it is **not branch-specific**, and it occurs under **coverage instrumentation** as well as a plain full run — so any reproduction attempt should include a coverage pass, which earlier targeted attempts did not.
