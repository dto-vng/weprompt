...
describe('built-in themes', () => {
  it('labels the official WePrompt themes clearly', () => {
    expect(BUILTIN_THEMES.find((theme) => theme.id === LIGHT_THEME_ID)?.name).toBe('WePrompt Light');
    expect(BUILTIN_THEMES.find((theme) => theme.id === DARK_THEME_ID)?.name).toBe('WePrompt Dark');
  });

  it('exposes only the official WePrompt themes for the normal theme screen', () => {
