...
3. Placeholder scan — BOTH checks must print nothing:
   ...
   - `officecli view <file> text | grep -iE 'northwind|connected sites|predictive quality|guided maintenance|line monitoring'` — these tokens exist only in the reference's sample content; any hit is a leftover you must replace or delete. Dividers and the closing slide are the most commonly forgotten — check them slide by slide.
